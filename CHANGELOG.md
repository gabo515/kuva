...
- **Typst tier for math in labels** (new opt-in `typst-math` feature) — labels containing `$...$` may now be typeset with the Typst compiler (linked as a library — no external binary) for real 2-D math: stacked fractions, radicals with vinculum, large operators, math italic. The whole label is compiled and embedded as an SVG fragment (SVG backend), pixmap (PNG), or through the PDF pipeline, with math color following the label color. `typst-math` is **not** part of `full`: it pulls a ~200-crate dependency tree and needs Rust >= 1.89, so `full` stays the core backend set (`embed_font` + `png` + `pdf`) and the typesetter is enabled explicitly with `cargo build --features typst-math`. The pre-built CLI binaries on the releases page do ship with it, since a downloaded binary cannot opt in afterwards. Builds on the always-on inline-Unicode lookup tier from 0.3.0, which remains the fallback everywhere: the terminal backend, builds without `typst-math`, and any label Typst fails to compile (one warning per distinct label). Bundles New Computer Modern Math (~0.75 MB compressed) as the math font. Layout reserves the fragment's measured extent rather than one nominal text line, via a new `ComputedLayout::x_label_math_extra` (and `ComputedLayout` is now `#[non_exhaustive]`, so future layout fields are not breaking additions), so a deep descender (the denominator of `$\frac{-b \pm \sqrt{b^2-4ac}}{2a}$`) clears the canvas bottom with the same gap ordinary text gets. Note: Typst math is not LaTeX (`mc` is one identifier — write `m c`). See *Reference → Math in Labels*.
- **Math in `TextPlot` bodies (typst tier)** — `$...$` inside a markdown body is spliced into the wrapped paragraph as a typeset fragment: kuva's own word-wrap treats each fragment as one unbreakable word (sized by its typeset width), surrounding words stay real selectable text in SVG/PDF, punctuation directly after math stays flush (`$x^2$.`), and a line holding a tall fragment (stacked fraction, radical) grows its leading so neighbouring lines never collide. Terminal output and builds without `typst-math` lower body math to inline Unicode exactly as before.
- **`TypstBackend`** (opt-in `typst` feature, zero deps; not in `full`) — emit a CETZ-based Typst document for external compilation with `typst compile`; `$...$` regions become native Typst math, typeset with fonts matching the surrounding document. Full primitive coverage: `Path` (arrowheads, chord ribbons, sankey flows, venn outlines) lowers to CETZ `merge-path` calls with arcs converted to cubic Béziers, and clip regions map to Typst's `box(clip: true)` — so quiver/network arrowheads and plot-area clipping survive the round trip. Library-only: the CLI's `-o` still accepts `.svg`/`.png`/`.pdf` only, so `.typ` is emitted by constructing a `TypstBackend` in Rust.
...
- **Force-directed (ggrepel-style) label placement.** A new `LabelStyle::Repel` variant runs a shared `render_utils::repel_labels` force-directed pass (labels pushed off each other and off the points in 2-D, with thin leader lines back to each). Wired into `VolcanoPlot` and `ManhattanPlot` (CLI `--label-style repel`), and `ScatterPlot` gains per-point labels for the first time: `.with_labels(iter)` + `.with_label_style(…)` / `.with_repel_labels()`, with CLI `kuva scatter --label-col <COL> --label-style <nudge|exact|repel>`. The Exact/Nudge/Arrow/Repel rendering is now one shared `draw_labels` path across all three plot types (deduplicating the previous per-plot copies).
- **Scatter marker shapes and LOESS smoothing.** `MarkerShape` gains `TriangleDown`, `Star`, `Pentagon`, and `Hexagon` (now 10 shapes), lifting every plot that reuses the marker set; a `MarkerShape::parse` helper and the CLI `kuva scatter --marker <shape>` flag expose them. A new `TrendLine::Loess { span }` variant (plus `ScatterPlot::with_loess()` / `with_loess_span(f)` and the shared `render_utils::loess` locally-weighted-regression helper) overlays a LOESS/LOWESS smoother for non-linear trends; CLI `--loess` / `--loess-span <F>`. LOESS takes precedence over `--trend` when both are given.

- **`StripPlot::with_horizontal(bool)`** — horizontal orientation (categories on the Y axis, values on X; seaborn `orient='h'`, ggplot2 `coord_flip`), completing the horizontal-orientation set alongside `BoxPlot`/`ViolinPlot`. Jitter and swarm spread run vertically within each row. The renderer already supported the swap for all three layout styles; this wires the field, `bounds()`, category-axis labels, and the CLI `kuva strip --horizontal` (both `--y` and `--group-col` modes) through to it.
- **`kuva strip --opacity <0..1>`** — CLI access to marker fill opacity (the existing `StripPlot::with_marker_opacity`). Semi-transparent markers reveal density where dense points overlap, which matters for real-data strip/beeswarm plots at large N.
...
- **`kuva funnel`: the color-mode flag is renamed `--color-by` → `--color-mode`** to remove a naming collision. Everywhere else `--color-by` selects a data column (`ColSpec`), but on `funnel` it chose a bar color *mode* (`uniform`/`stage`/`gradient`). It is now `--color-mode`, with `--color-by` kept as a hidden backward-compatible alias so existing scripts keep working.

### Fixed

- **All-numeric column *names* could not be selected on the CLI** ([#109](https://github.com/Psy-Fer/kuva/issues/109)) — `--x 2024` parses to `ColSpec::Index(2024)`, so a header column literally named `2024` (a year, etc.) was unreachable and failed with an out-of-range error. `DataTable::resolve` now falls back to a header column named by that number when the index is out of range; in-range indices are untouched, so `--x 0` still always means the first column (fully backward compatible). Applied to the TSV/CSV and parquet-projection paths, and the auto axis/series labels (`scatter`/`line`/`twin-y`/`parallel`/`radar`) route through the shared resolver so the label shows the real column name.
  - Added explicit disambiguation prefixes on any column argument, parsed centrally in `ColSpec::from_str` so they work across every subcommand: `name:X` / `col:X` force a column name (even an all-digit one, e.g. select a column literally named `1` via `--x name:1`), and `idx:N` / `index:N` / `#N` force a pure 0-based index that never resolves to a name (an out-of-range `idx:` errors instead of falling back). A bare token keeps the index-first-with-fallback heuristic.
  - Added **column ranges and globs** on multi-column arguments (`--y`, `--value-cols`, `--samples`, `--axes`): index ranges `a..b` / `a..=b` / `a..` / `..b` / `..`, and header-name globs `sum_*` / `*_A` / `*mid*` (`*` matches any run). They expand against the parsed table (deduped, order-preserving) via `DataTable::expand_columns`; a lone range/glob still routes into the plot's multi-column path. Wired through scatter/line/bar/histogram/box/violin/strip/density/ridgeline/parallel/radar/coverage.
