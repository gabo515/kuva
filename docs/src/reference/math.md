...
- **Typst tier** (feature `typst-math`): the whole label is typeset by the
  [Typst](https://typst.app) compiler (linked as a library, no external
  binary) and embedded in SVG/PNG/PDF output as real 2-D math: stacked
  fractions, radicals with vinculum, large operators, math italic. The
  terminal always uses the lookup tier (a character grid can't hold typeset
  math).

  `typst-math` is opt-in and is **not** part of `full`: it pulls a large
  dependency tree (the Typst compiler) and needs Rust 1.89 or newer, so you
  enable it explicitly with `--features typst-math`. The pre-built CLI
  binaries from the releases page do ship with it, since a downloaded binary
  has no way to opt in afterwards.
...
With the `typst-math` feature, `$...$` labels upgrade automatically from the
inline Unicode above to real typeset math. A compile failure never breaks a plot:
...
Generated by `cargo run --features full,typst-math --example math`, so these
show the **typst tier**. Without `typst-math` the same code produces the
lookup tier's inline-Unicode forms instead:

### Scientific axes

The standard RNA-seq and GWAS axis pair, `$\log_2$ fold change` /
`$-\log_{10}(p)$`:
